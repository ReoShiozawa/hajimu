...
> 🇺🇸 **English version available:** [PLUGIN_DEVELOPMENT.md](PLUGIN_DEVELOPMENT.md)
...
| `hajimu_numeric_from_f64_copy(data, length)` | `double *` から数値ベクトルをコピーして返す | `return hajimu_numeric_from_f64_copy(xs, n);` |
| `hajimu_matrix_from_f64_copy(data, rows, cols)` | row-major の `double *` から数値行列をコピーして返す | `return hajimu_matrix_from_f64_copy(xs, r, c);` |
...
static Value 処理(int argc, Value *argv) {
    ...
    int    len = argv[0].string.byte_length;    // 文字列長
    ...
    // 数値ベクトル / 行列のバッファは借用です。保持したい場合はコピーしてください。
    // hajimu_*_f64_data は dtype が NUMERIC_DTYPE_F64 の場合だけ有効です。
