...
数値ベクトルは、数値だけを連続メモリで保持する研究計算向けの型です。通常の `配列` より柔軟性は低い一方、合計・平均・内積などを C 側で一括処理できます。

| 関数 | 説明 |
|---|---|
| `ベクトル(配列)` | 数値配列から数値ベクトルを作成 |
| `配列化(ベクトル)` | 数値ベクトルを通常配列へ変換 |
| `データ型(ベクトル)` | 数値ベクトルの dtype を返す。初期値は `"f64"` |
| `データ型サイズ(ベクトルまたはdtype名)` | dtype の論理要素サイズをバイト数で返す |
| `論理バイト数(ベクトル)` | dtype に基づく論理バイト数を返す |
| `保存バイト数(ベクトル)` | 現在の内部保存に使うバイト数を返す |
| `型変換(ベクトル, dtype)` | `"f64"` / `"f32"` / `"i64"` / `"i32"` / `"bool"` へ変換した数値ベクトルを返す |
| `ゼロ配列(長さ)` | 0 で埋めた数値ベクトルを作成 |
| `一配列(長さ)` | 1 で埋めた数値ベクトルを作成 |
| `範囲ベクトル(終了)` | `0` から `終了` 未満の数値ベクトル |
| `範囲ベクトル(開始, 終了 [, ステップ])` | 半開区間の数値ベクトル |
| `ベクトル合計(ベクトル)` | 合計 |
| `平均(ベクトル)` | 平均 |
| `分散(ベクトル)` | 分散 |
| `標準偏差(ベクトル)` | 標準偏差 |
| `分位点(ベクトル, q)` | `q` を 0 から 1 で指定する線形補間分位点 |
| `中央値(ベクトル)` | 中央値 |
| `標準化(ベクトル)` | 平均 0・標準偏差 1 に正規化した数値ベクトル |
| `Zスコア(ベクトル)` | `標準化` と同じ |
| `ノルム(ベクトル [, p])` | p ノルム。省略時は L2 |
| `最小最大スケール(ベクトル)` | 最小値を 0、最大値を 1 に線形変換 |
| `クリップ(ベクトル, 最小, 最大)` | 各要素を指定範囲に収める |
| `共分散(ベクトル1, ベクトル2)` | 母共分散 |
| `相関(ベクトル1, ベクトル2)` | Pearson 相関係数 |
| `ヒストグラム(ベクトル, ビン数)` | `counts` / `edges` を含むヒストグラム辞書 |
| `訓練テスト分割(ベクトルまたは行列, テスト比率)` | 先頭を訓練、末尾をテストとして分割 |
| `欠損削除(ベクトルまたは行列)` | `NaN` を含む要素または行を削除 |
| `欠損補完(ベクトルまたは行列, 値)` | `NaN` を指定値で置換 |
| `NaNか(値)` | 数値が `NaN` か判定 |
| `平均二乗誤差(実測, 予測)` | MSE |
| `平均絶対誤差(実測, 予測)` | MAE |
| `決定係数(実測, 予測)` | R2 score |
| `正解率(実測, 予測)` | 0.5 を閾値とした accuracy |
| `適合率(実測, 予測)` | 0.5 を閾値とした precision |
| `再現率(実測, 予測)` | 0.5 を閾値とした recall |
| `F1スコア(実測, 予測)` | 0.5 を閾値とした F1 score |
| `混同行列(実測, 予測)` | `[[TN, FP], [FN, TP]]` の 2x2 行列 |
| `最大(ベクトル)` | 最大値 |
| `最小(ベクトル)` | 最小値 |
| `ベクトル加算(ベクトル, 数値またはベクトル)` | 要素ごとの加算 |
| `ベクトル減算(ベクトル, 数値またはベクトル)` | 要素ごとの差分 |
| `ベクトル乗算(ベクトル, 数値またはベクトル)` | 要素ごとの乗算 |
| `ベクトル除算(ベクトル, 数値またはベクトル)` | 要素ごとの除算 |
| `ベクトル絶対値(ベクトル)` | 要素ごとの絶対値 |
| `ベクトル平方根(ベクトル)` | 要素ごとの平方根 |
| `ベクトル正弦(ベクトル)` | 要素ごとの sin |
| `ベクトル余弦(ベクトル)` | 要素ごとの cos |
| `ベクトル対数(ベクトル)` | 要素ごとの自然対数 |
| `内積(ベクトル1, ベクトル2)` | 内積 |
| `要約(ベクトル)` | 件数・平均・標準偏差・最小・最大を辞書で返す |
| `数値ベクトルか(値)` | 数値ベクトルかどうか判定 |

英語 alias: `vector`, `to_array`, `dtype`, `dtype_size`, `nbytes`, `storage_bytes`, `astype`, `zeros`, `ones`, `range_vector`, `vector_sum`, `mean`, `variance`, `std`, `quantile`, `median`, `normalize`, `z_score`, `norm`, `minmax_scale`, `clip`, `covariance`, `correlation`, `histogram`, `train_test_split`, `drop_missing`, `fill_missing`, `is_nan`, `mse`, `mae`, `r2_score`, `accuracy`, `precision`, `recall`, `f1_score`, `confusion_matrix`, `max`, `min`, `vector_add`, `vector_sub`, `vector_mul`, `vector_div`, `vector_abs`, `vector_sqrt`, `vector_sin`, `vector_cos`, `vector_log`, `dot`, `describe`, `is_vector`

現在の dtype は実際の保存バッファにも反映されます。`f64` は `double`、`f32` は `float`、`i64` / `i32` は整数、`bool` は 1 バイト値として保持されます。`i32` は範囲外を `INT32_MIN` / `INT32_MAX` に収め、`bool` は `0` または `1` になります。長さの違うベクトル同士の演算、0 除算、平方根・対数の定義域外入力では、どの演算でどの要素が問題になったか分かる実行時診断を出します。
...
表示(中央値(x))        // 2.5
表示(内積(x, x))       // 30
表示(共分散(x, ベクトル([2, 4, 6, 8]))) // 2.5
表示(相関(x, ベクトル([2, 4, 6, 8]))) // 1
表示(ヒストグラム(x, 2)["counts"]) // [2, 2]
表示(文字列化(ベクトル乗算(x, 2))) // [2, 4, 6, 8]
x[0] = 10
表示(x[0])             // 10
表示(JSON化(x))        // [10,2,3,4]
表示(配列化(x)[0])     // 10

変数 xi = 型変換(ベクトル([1.9, 2.1]), "i32")
表示(データ型(xi))      // i32
表示(xi[0])             // 1
表示(論理バイト数(xi))  // 8
表示(保存バイト数(xi))  // 8
...
表示(文字列化(y))      // [0, 1, 2, 3, 4]
表示(ノルム(ベクトル([3, 4]))) // 5
表示(最小最大スケール(ベクトル([10, 20, 30]))) // [0, 0.5, 1]
```

### 数値行列

数値行列は、2 次元の数値データを行優先の連続メモリで保持します。研究計算・機械学習の入力データ、線形代数、特徴量行列の土台として使います。

| 関数 | 説明 |
|---|---|
| `行列(配列)` | 2 次元配列または数値ベクトル配列から行列を作成 |
| `データ型(行列)` | 数値行列の dtype を返す |
| `型変換(行列, dtype)` | 指定 dtype 相当へ変換した数値行列を返す |
| `論理バイト数(行列)` | dtype に基づく論理バイト数を返す |
| `保存バイト数(行列)` | 現在の内部保存に使うバイト数を返す |
| `形状(行列)` | `[行数, 列数]` を返す |
| `行列取得(行列, 行, 列)` | 要素を取得 |
| `行列設定(行列, 行, 列, 値)` | 要素を設定 |
| `行取得(行列, 行)` | 指定行を数値ベクトルとして取得 |
| `列取得(行列, 列)` | 指定列を数値ベクトルとして取得 |
| `転置(行列)` | 転置行列。内部では共有バッファ view と stride 入れ替えを使う |
| `行列積(左, 右)` | 行列積 |
| `行列加算(左, 右)` | 同じ形の行列を要素ごとに加算 |
| `行列減算(左, 右)` | 同じ形の行列を要素ごとに減算 |
| `行列スケール(行列, 数値)` | 行列の全要素に数値を掛ける |
| `行列要素積(左, 右)` | 同じ形の行列の Hadamard 積 |
| `単位行列(サイズ)` | 単位行列を作成 |
| `行列式(行列)` | 正方行列の行列式 |
| `逆行列(行列)` | 正方行列の逆行列 |
| `線形方程式を解く(係数行列, 右辺)` | `Ax = b` を解く。右辺は数値ベクトルまたは行列 |
| `線形回帰(特徴量行列, 目的変数 [, 切片あり])` | 最小二乗法の線形回帰モデル辞書を返す |
| `線形予測(モデル, 特徴量)` | `線形回帰` のモデルで単体ベクトルまたは行列を予測 |
| `k平均法(行列, k [, 反復回数])` | k-means クラスタリング。`centers` / `labels` を返す |
| `k近傍予測(訓練特徴量, 訓練ラベル, 入力, k)` | k-nearest neighbors。入力は数値ベクトルまたは行列 |
| `ロジスティック回帰(特徴量行列, ラベル [, 学習率] [, 反復回数])` | 2値分類用ロジスティック回帰モデル |
| `ロジスティック予測(モデル, 特徴量)` | ロジスティック回帰の確率予測 |
| `ロジスティック分類(モデル, 特徴量 [, 閾値])` | ロジスティック回帰の確率を 0/1 ラベルに変換 |
| `行列か(値)` | 数値行列かどうか判定 |
| `配列化(行列)` | 通常の 2 次元配列へ変換 |
| `CSV読込(パス [, ヘッダーあり])` | 汎用 CSV を読む。既定では先頭行をヘッダーとして、各行を辞書で返す。`偽` の場合は行配列の配列を返す |
| `CSV列(行配列, 列名または列番号)` | `CSV読込` の結果から列を取り出す。辞書行には列名、配列行には列番号を指定 |
| `JSON行読込(パス [, 最大行数])` / `JSONL読込` | JSON Lines を行ごとに解析して配列で返す。既定の最大行数は 100000 |
| `CSV数値読込(パス [, ヘッダーあり] [, missing mode])` | 数値だけの CSV を行列として読み込む。missing mode は `"error"` / `"nan"` / `"zero"` |
| `TSV数値読込(パス [, ヘッダーあり] [, missing mode])` | 数値だけの TSV を行列として読み込む |
| `要約(行列)` | 列ごとの要約統計を配列で返す |

英語 alias: `matrix`, `dtype`, `astype`, `nbytes`, `storage_bytes`, `shape`, `matrix_get`, `matrix_set`, `matrix_row`, `matrix_column`, `transpose`, `matmul`, `matrix_add`, `matrix_sub`, `matrix_scale`, `matrix_hadamard`, `identity`, `determinant`, `inverse`, `solve_linear`, `solve`, `linear_regression`, `predict_linear`, `kmeans`, `knn_predict`, `logistic_regression`, `predict_logistic`, `predict_logistic_class`, `read_csv`, `csv_column`, `read_json_lines`, `read_csv_numeric`, `read_tsv_numeric`, `describe`, `is_matrix`, `to_array`

行列積の形が合わない場合、行・列インデックスが範囲外の場合、CSV の列数が途中で変わる場合、数値として読めないセルがある場合は、行列サイズや CSV の行・列番号を含む診断を出します。
...
表示(行列取得(c, 0, 0)) // 22
表示(列取得(c, 0))      // [22, 49]
表示(データ型(型変換(c, "f32"))) // f32
表示(行列スケール(c, 0.5))
表示(行列要素積(c, c))
表示(行列式(行列([[1, 2], [3, 4]]))) // -2
表示(逆行列(行列([[4, 7], [2, 6]])))
表示(線形方程式を解く(行列([[2, 1], [1, 3]]), ベクトル([1, 2])))

変数 model = 線形回帰(行列([[0], [1], [2], [3]]), ベクトル([1, 3, 5, 7]))
表示(model["weights"])          // [2]
表示(model["intercept"])        // 1
表示(線形予測(model, ベクトル([4]))) // 9

行列設定(c, 0, 0, 999)
表示(c[0][0])          // 999
表示(JSON化(c))        // [[999,28],[49,64]]

変数 data = CSV数値読込("data.csv", 真) // 先頭行をヘッダーとしてスキップ
表示(平均(列取得(data, 0)))
表示(要約(data)[0]["mean"])

変数 rows = CSV読込("people.csv")
表示(rows[0]["name"])
表示(CSV列(rows, "score"))

変数 分割 = 訓練テスト分割(data, 0.2)
表示(形状(分割["train"]))
表示(形状(分割["test"]))

変数 シャッフル分割 = 訓練テスト分割(data, 0.2, 123) // seed 指定で再現可能
```
...
| 定数 | 説明 | 例 |
|---|---|---|
| `システム名` / `system_name` | OS名 | `"macOS"` / `"Linux"` / `"Windows"` |
| `アーキテクチャ` / `architecture` / `arch` | CPUアーキテクチャ | `"arm64"` / `"x86-64"` |
| `はじむバージョン` / `hajimu_version` | 実行系バージョン | `"1.5.0"` |
| `システム["OS"]` / `system["os"]` | `システム名` と同じ | |
| `システム["区切り文字"]` / `system["path_separator"]` | パス区切り文字 | `/`、Windowsでは `\` |
| `システム["改行"]` / `system["newline"]` | 改行文字 | `\n`、Windowsでは `\r\n` |
...
不正な JSON、閉じ括弧不足、末尾に余分な文字がある入力は `無` を返します。
...
### JSON行読込(パス [, 最大行数]) / JSONL読込(パス [, 最大行数])

JSON Lines 形式のファイルを、非空行ごとに JSON として解析し、配列で返します。
既定の最大行数は `100000` です。不正な行がある場合は行番号付きのエラーになります。

```hajimu
変数 rows = JSON行読込("events.jsonl")
表示(rows[0]["type"])
```
...
ジェネレータを別の変数へ代入した場合、読み取り位置を含む同じ状態を共有します。
...
`1` のような整数値と `1.5` のような小数値は、内部ではどちらも数値として扱われますが、
`型判定(値, "整数")` と `型判定(値, "小数")` で区別できます。
配列・文字列のインデックスには整数値が必要です。

---

## UTF-8 文字列長

文字列値は内部で UTF-8 のバイト数と文字数を分けて管理します。
`長さ(文字列)` はバイト数ではなく文字数を返します。
