...
static ASTNode *english_for_statement(Parser *parser);
...
static bool is_reserved_name_token(TokenType type);
static const char *reserved_name_tip(const Token *token);
static void consume_block_intro(Parser *parser, TokenType keyword, const char *message);
static void consume_end(Parser *parser);
...
static void consume(Parser *parser, TokenType type, const char *message) {
    ...
    if (parser->current.type == TOKEN_EOF) {
        ...
        if (type == TOKEN_IDENTIFIER && is_reserved_name_token(parser->current.type)) {
            snprintf(msg, sizeof(msg),
                     "%s。'%s' は予約語なので名前に使えません。%s",
                     message, tok_repr, reserved_name_tip(&parser->current));
        } else {
...
static bool token_text_equals(const Token *token, const char *text) {
    ...
    return token->length == (int)length && memcmp(token->start, text, length) == 0;
}
...
static const char *reserved_name_tip(const Token *token) {
    if (token_text_equals(token, "class") || token_text_equals(token, "型")) {
        return "クラスに関係する値なら class_name や 型名 など、意味が分かる別名にしてください。";
    }
    if (token_text_equals(token, "function") || token_text_equals(token, "fn") ||
        token_text_equals(token, "関数")) {
        return "関数そのものを入れる変数なら handler や callback などの名前が使えます。";
    }
    if (token_text_equals(token, "str") || token_text_equals(token, "string") ||
        token_text_equals(token, "文字列")) {
        return "文字列値なら text、message、name など具体的な名前にしてください。";
    }
    if (token_text_equals(token, "number") || token_text_equals(token, "num") ||
        token_text_equals(token, "数値")) {
        return "数値なら count、total、score など役割が分かる名前にしてください。";
    }
    if (token_text_equals(token, "list") || token_text_equals(token, "array") ||
        token_text_equals(token, "配列")) {
        return "配列なら items、rows、entries など中身が分かる名前にしてください。";
    }
    if (token_text_equals(token, "if") || token_text_equals(token, "else") ||
        token_text_equals(token, "for") || token_text_equals(token, "while")) {
        return "制御構文の単語は使えません。条件や処理内容を表す名前にしてください。";
    }
    return "末尾に _value を付ける、または用途が分かる名前へ変えてください。";
}

static void consume_block_intro(Parser *parser, TokenType keyword, const char *message) {
    if (match(parser, keyword)) return;
    if (match(parser, TOKEN_COLON)) return;
    consume(parser, keyword, message);
}

static void consume_end(Parser *parser) {
    consume(parser, TOKEN_END, "'終わり' または 'end' が必要です");
}
...
static ASTNode *statement(Parser *parser) {
    ...
    if (match(parser, TOKEN_FOR)) {
        return english_for_statement(parser);
    }
...
static ASTNode *var_declaration(Parser *parser, bool is_const) {
    ...
    // 型注釈（現時点では実行時型チェックを持たないため、構文として受け付ける）
    if (match(parser, TOKEN_TYPE_IS)) {
...
static ASTNode *if_statement(Parser *parser) {
    ...
    // なら
    consume_block_intro(parser, TOKEN_THEN, "'なら' または 'then' が必要です");
...
static ASTNode *while_statement(Parser *parser) {
    ...
    // の間
    consume_block_intro(parser, TOKEN_WHILE_END, "'の間'、'do'、または ':' が必要です");
...
static ASTNode *english_for_statement(Parser *parser) {
    ...
    consume(parser, TOKEN_IDENTIFIER, "for の後にループ変数名が必要です");
    char *var_name = copy_token_string(&parser->previous);

    if (match(parser, TOKEN_IN)) {
        ASTNode *iterable = expression(parser);
        consume(parser, TOKEN_COLON, "for item in items の後に ':' が必要です");
        ASTNode *body = block(parser);
        consume_end(parser);
    ...
    if (match(parser, TOKEN_FROM)) {
        ASTNode *start = expression(parser);
        consume(parser, TOKEN_TO, "for i from 開始値 to 終了値 の 'to' が必要です");
        ASTNode *end = expression(parser);
        consume(parser, TOKEN_COLON, "for i from 開始値 to 終了値 の後に ':' が必要です");
        ASTNode *body = block(parser);
        consume_end(parser);
        ASTNode *node = node_for(var_name, start, end, NULL, body, line, column);
        free(var_name);
        return node;
    }

    error(parser, "for 文は 'for item in items:' または 'for i from 1 to 10:' の形で書いてください");
    free(var_name);
...
static ASTNode *import_statement(Parser *parser) {
    ...
    // エイリアス: 「として 名前」または英語構文の「as name」
    char *alias = NULL;
    if ((check(parser, TOKEN_IDENTIFIER) && token_text_equals(&parser->current, "として")) ||
        (check(parser, TOKEN_TYPE_IS) && token_text_equals(&parser->current, "as"))) {
        advance(parser);  // 「として」または「as」を消費
